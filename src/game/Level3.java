...
public class Level3 extends GameLevel {
    ...
    /**
     * Initialises new Level3
     *
     * @param game game implementing this level
     */
    public Level3(Game game) {
        super.populate(game);
        game.view = new Background(this, 1000, 700, 3);
    ...
    public void compare(String name, int comparisonIndex, String type) {
        ...
        if (type.equals("meteorite")) {
            if (!(this.meteoriteList.get(comparisonIndex).getName()).equalsIgnoreCase(name)) {
                playCollisionMusic(false);
                System.out.println("Oops! That shoud have been " + this.meteoriteList.get(comparisonIndex).getName());
                correctGuesses = 0;
                if (rocket.getLives() == 1) {
                    this.getGame().gameOver();
                }
            ...
            if (correctGuesses == 3) {
                rocket.numberOfCorrectGuesses++;
                System.out.println("number of correct guesses " + rocket.numberOfCorrectGuesses);
                if (rocket.numberOfCorrectGuesses == 6) {
                    this.destroyRowMeteorites();
                    System.exit(0);
                }
                correctGuesses = 0;
            }
    ...
    public boolean isCompleted() {
        ...
        return this.getPlayer().numberOfCorrectGuesses == 3;
    }

    @Override
    public void playBackgroundMusic() {
        try {
            gameMusic = new SoundClip("data/3rdlevelmusic.mp3");
            gameMusic.loop();
